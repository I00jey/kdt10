...
-- 데이터베이스 : 데이터의 집합
-- DBMS : 데이터베이스를 운영/관리하는 프로그램 (mySQL)
-- 테이블 : 하나 이상의 열과 행로 구성된 데이터베이스 최소 단위
-- SQL : RDBMS에서 사용되는 언어
-- 참고! SQL은 대소문자를 구별하지 않음
-- 명령어를 구분하기 쉽게 하기위해서 대문자로 작성하는 것
-- 단, 데이터베이스명, 테이블명 같은 경우는 윈도우에서는 대소문자 구분 X
-- 그 외, 리눅스 환경의 경우에서는 대소문자 구분 (주의 필요)
...
-- 제약조건("옵션")
-- NOT NULL : NULL을 허용 X
-- AUTO_INCREMENT : 자동으로 숫자 증가, 테이블의 데이터가 추가될 때마다 자동으로 숫자가 증가
