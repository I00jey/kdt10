...
// - password를 sha512 알고리즘 방식으로 암호화
...
console.log(createHashedPW("1234")); // 같은 pw라면 같은 해시값 출력
...
const createPbkdf = (password) => {
    ...
    // hash 생성 => salt & password를 결합해서 해시 생성
    const hash = crypto
...
function checkPW(inputPW, savedSalt, savedHash) {
    ...
    return savedHash === newhash; // 같으면 true, 다르면 false
}

// 사용 예제
const password = "1234!";
// 비번 암호화
const { salt, hash } = createPbkdf(password);
